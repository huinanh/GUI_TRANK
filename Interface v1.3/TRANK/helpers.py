...
def nk_plot( nkf, lamda_fine,  lamda_list = [], title_string = '', file_name = 'nk.pdf', show_nodes = False, zoom_window = [], 
			show_plots = False, fig=None, if_e=False):
	...
	if if_e:
		e_real=nk_smooth.real**2-nk_smooth.imag**2
		e_imag=2*nk_smooth.real*nk_smooth.imag
		nax.plot(lamda_smooth, e_real,   linewidth = 2.0, color = 'c',  linestyle = '-', zorder = 0.8,label='e.real')
		kax.plot(lamda_smooth, e_imag,   linewidth = 2.0, color = 'm', linestyle = '-', zorder = 0.8,label='e.imag')
		kax.legend(loc='upper right')
		nax.legend(loc='upper left')
	...
	### fix limits
	ylim =nax.get_ylim()
	nax.set_ylim(0,ylim[1]*1.05)
	ylim =kax.get_ylim()
	kax.set_ylim(0,ylim[1]*1.05)
